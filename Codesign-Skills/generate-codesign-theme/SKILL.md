---
name: generate-codesign-theme
description: >
  从腾讯 CoDesign 设计稿中采集设计令牌并生成完整的 DESIGN.md 设计规范文档 + Tailwind CSS 主题文件。
  当用户提供 CoDesign URL 并要求"提取主题"、"生成设计规范"、"导出设计系统"、
  "提取这个设计稿的设计令牌"、"帮我做一个主题"、"分析这个设计稿的设计风格"时使用。
...
从 CoDesign 设计稿采集设计令牌，生成 DESIGN.md 设计规范 + Tailwind CSS 主题。

## 前置条件

- Node.js >= 18
- 已安装 `extract-codesign-data` 技能（用于数据采集）
- 可选：CoDesign API token（获取更精确的设计令牌）

## 与网页主题生成的差异

| 维度 | 网页 (`generate-theme`) | CoDesign 设计稿 |
|------|--------------------------|---------------|
| 数据源 | 浏览器渲染的 DOM | Canvas 渲染 + 可选 API |
| theme.json 准确性 | 高（从 computedStyle 提取） | 中/低（Canvas 渲染可能无法提取完整 DOM） |
| 截图重要性 | 高 | **极高**（Canvas 渲染时唯一可靠来源） |
| API 支持 | 无 | 有（CoDesign Open API，推荐） |
| 设计令牌来源 | CSS 变量 + computedStyle | API 直接提供 或 截图分析 |

**核心原则：** CoDesign 设计稿使用 Canvas/WebGL 渲染，浏览器模式下的 theme.json 可能不完整。**截图是最终真相，比任何机器提取的数据都重要。**

## 工作流

### Step 1: 采集设计数据 + 截图

使用 `extract-codesign-data` 技能采集 theme.json + 多维度截图：

```bash
# 方式 A: 浏览器模式（快速，但可能不完整）
node <extract-codesign-data>/scripts/extract.mjs <codesign-url> --theme --screenshot --scroll -o ./theme-data

# 方式 B: API 模式（推荐，数据更精确）
set CODESIGN_TOKEN=your_token
node <extract-codesign-data>/scripts/extract.mjs <project-id> --all --use-api -o ./theme-data

# 方式 C: 手动截图（最可靠）
# 在 CoDesign 中打开设计稿，截图保存为 theme-data/screenshot.png
```

采集后 `theme-data/` 中应有：
- `theme.json` — 设计令牌（颜色、字体、间距、圆角、阴影、动画…）
- `screenshot.png` — **全页截图（最重要的参考）**
- `data.json` — 组件树（API 模式时可用）
- `notes.json` — 组件标注（API 模式时可用）

### Step 2: 截图分析（最关键的一步）

**截图是设计规范的最终真相**。CoDesign 设计稿的很多关键信息只存在于视觉呈现中，无法从 CSS 属性提取：
...
| 📱 响应式策略 | 不同视口下的布局变化（需在 CoDesign 中切换） | ❌ 无法 |
...
2. **看组件标注** `notes.json`（如果 API 模式可用）：
   - 每个组件的设计说明
   - 交互状态说明
   - 尺寸和间距标注

3. **看组件树** `data.json`（如果 API 模式可用）：
   - 页面结构层级
   - 组件类型和数量
   - 组件之间的嵌套关系
...
读取 `theme-data/theme.json`，对每个 token 类别进行分析和归类：
...
**⚠️ 注意：** CoDesign Canvas 渲染模式下，theme.json 的数据可能不完整或有偏差。当 theme.json 的值与截图中观察到的视觉效果冲突时，**以截图为准**。

**API 模式的优势：**
如果有 CoDesign API token，通过 `--use-api` 模式获取的设计令牌更准确，因为直接从设计稿数据源获取，而不是从渲染后的 DOM 提取。
...
| 名称 | 字体 | Tailwind | 用途 |
|------|------|---------|----- |
| Sans | [从截图+theme.json 推断], system-ui, sans-serif | `font-sans` | 正文、UI 组件 |
...
:root {
  ...
  --primary: <从 theme.json colors.text[1] 或 API 数据推断>;
...
6. **CoDesign 特有：** 如果使用了 API 模式，优先使用 API 返回的设计令牌，因为它们更准确
...
| 优先级 | 数据来源 | 提供什么 | 可信度 |
|--------|---------|---------|--------|
| ⭐⭐⭐⭐⭐ | **截图（全页 + 组件）** | 品牌调性、布局节奏、视觉层级、装饰元素、组件形态 | 最高 — 视觉真相 |
| ⭐⭐⭐⭐ | **CoDesign API 数据**（`data.json`, `notes.json`） | 精确的组件树、标注、设计令牌 | 高 — 直接来自数据源 |
| ⭐⭐⭐ | **cssVariables（:root 变量）** | 原始设计系统的命名和值 | 高 — 开发者意图 |
| ⭐⭐ | **theme.json（颜色/字体/间距/阴影）** | 属性值的统计频率 | 中 — 机器提取，Canvas 模式下可能不完整 |

## 提示

- **截图是最终真相** — 当 token 数据和截图冲突时，**永远以截图为准**。CoDesign Canvas 渲染模式下，截图是唯一可靠的数据源
- **先看截图再读数据** — 工作流中截图分析在 theme.json 分析之前，这是故意的。先建立视觉直觉，再用数据验证
- **API token 很重要** — 如果有 CoDesign API token，使用 `--use-api` 模式可以获取更精确的设计令牌，绕过 Canvas 渲染限制
- **cssVariables 是金矿** — 如果设计稿定义了 CSS 变量，优先使用它们，因为这是原始设计系统的意图
...
- **CoDesign 特有：** 如果浏览器模式提取的 theme.json 数据不完整，可以手动从截图中提取关键颜色值，写入 DESIGN.md 和 globals.css
