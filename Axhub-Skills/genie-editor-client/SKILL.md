...
description: 处理 Genie Editor 宿主侧客户端接入工作流，包括调整面板可编辑项、页面级属性聚合、多方案设计比稿与方案切换，以及页面资源上下文与定位信息补齐。当用户要让元素或页面“能在调整面板改”、按页面统一整理可编辑项、先出多版方向再切换对比，或补页面定位信息以便调试时快速找到当前页和对应实现时使用。
---
...
- tweak / schema / values / adapter / update / card 方案切换优先复用 `@axhub/genie-editor-client`
- 页面资源上下文 / `filePath` / `targetPath` / `projectPath` 优先走 `createGenieEditor(...).host.getResourceContext()` 或 `window.__GENIE_EDITOR_HOST__`
